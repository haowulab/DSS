...
showOneDMR <- function(OneDMR, BSobj, ext=500, ylim) {
    ...
    for(i  in 1:ncol(X)) {
        ...
        ## black curve, total number, normalized to 0~1
        thisN.norm=thisN[ix1,i]/max(thisN[ix1,])
        lines(thispos[ix1], thisN.norm, type="l", col="black", lwd=1)
        axis(side=4, at=seq(0,1,length.out=5), labels=round(seq(0, max(thisN[ix1,]), length.out=5)) )
