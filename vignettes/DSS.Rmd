...
The parallelism is achieved by specifying the `BPPARAM` parameter in `DMLtest` function,  
through the functionalities provided in  `r Biocpkg("BiocParallel")` package. 
Please see the help for `DMLtest` function for more description and example codes. 
To know more about the `r Biocpkg("BiocParallel")`, please read its documentations.
...
```{r fig2, echo=FALSE, fig.cap="Parallelization in DML calling", out.width = '100%'}
knitr::include_graphics("parallel.pdf")
```
