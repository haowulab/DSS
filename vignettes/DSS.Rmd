...
(the codes require installation of ``GenomicFeatures``, ``Rsamtools``,
and ``GenomicRanges`` packages).

- Sequence alignment. There are several RNA-seq aligner, for example, **tophat**
or  **STAR**. Assume the alignment result is saved in a BAM file ``data.bam``.

- Choose a gene annotation. ``GenomicFeatures`` package provides a convenient way to
...
- Obtain count table based on the alignment results and gene annotation.
This can be done in several steps. First read in the BAM file using the ```Rsamtools``` package:
```{r eval=FALSE}
...
There are other ways to obtain the counts, for example, using ``RSEM``, 
or Bioconductor packages like ``QuasR`` or ``easyRNASeq``. 
...
DSS depends on ``bsseq`` Bioconductor package, which has neat definition of
...
using ``bismark`` (version 0.10.0, commands for newer versions could be different)
for BS-seq alignment and count extraction. These steps require installation
of ``bowtie`` or ``bowtie2``, ``bismark``, and the fasta file for reference genome.
...
```{r fig1, echo=FALSE, fig.cap="An example DMR", out.width = '100%'}
knitr::include_graphics("aDMR.pdf")
```
