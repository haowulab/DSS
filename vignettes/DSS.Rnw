...
using {\tt bismark} for BS-seq alignment and count extraction. These steps require installation
...
through ordinary least square on transformed methylation levels. Standard errors
...
These procedures are computationally very efficient. For a typical RRBS dataset with 4 million CpG sites,
it takes around 20 minutes. In comparison, other methods such as RADMeth or BiSeq
...
\newblock {\em Bioinformatics\/}~\textbf{In revision.}
